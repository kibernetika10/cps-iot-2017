...
http.listen(8080);
//var clientIpAddress = io.sockets.request.socket.remoteAddress;
    ...
    timeout = setTimeout(function() { // this part of code will be run after 50 ms; if in-between input changes above code clears it
        ...
        if (last_value != last_sent) { // to send only on value change
           ...
           //if (~idx && ~clientIpAddress.indexOf('.')) address4 = clientIpAddress.slice(idx + 1);
            ...
            io.sockets.emit("messageToClient", "socket.request.connection._peername.family: " + io.sockets.request.connection._peername.family);
            io.sockets.emit("messageToClient", "socket.request.connection._peername.port: " + io.sockets.request.connection._peername.port);*/
        }
